...
/**
 * filters the entries of an object by their values
 * and returns an array of the keys matching the condition
 * e.g. filterKeys({ a: 1, b: 2 }, 1) returns [a]
 */
export function filterKeys<K extends string, V>(
  object: Record<K, V>,
  filter: V | ((value: V) => boolean)
): K[] {
  let filterFunction: (value: V) => boolean;
  if (typeof filter === "function") {
    filterFunction = filter as (value: V) => boolean;
  } else {
    filterFunction = (value: V) => value === filter;
  }

  return (Object.values(object) as [K, V][])
    .filter(([_key, value]) => filterFunction(value))
    .map(([key, _value]) => key);
}
